...
class LokiClient:
    ...
    async def query_logs(
        self, logql: str, limit: int = 100, since: str = "1h"
    ) -> list[dict]:
    ...
    async def get_namespace_errors(
        ...
        query = (
            f'{{namespace="{namespace}"}} |~ "(?i)(error|exception|fatal|panic|crash)"'
        )
        entries = await self.query_logs(query, limit=limit, since=since)
        if not entries:
    ...
    async def search_logs(
        ...
        if not entries:
            return (
                f'No logs matching "{query_text}" found in {scope} in the last {since}.'
            )
        return self._format_entries(entries)
    ...
    async def get_log_volume(self, namespace: str, since: str = "1h") -> dict:
        ...
        try:
            ...
            logger.warning(
                "Loki log volume query failed", namespace=namespace, error=str(exc)
            )
